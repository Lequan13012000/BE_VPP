    ...
    public class OrdersController : ControllerBase
    {
        ...
        public IActionResult Get()
        {
            try
            {
                ...
                {
                    var sqlcommand = "SELECT * FROM Orders ORDER BY order_date DESC";
        ...
        public IActionResult Pagination(int PageNumber, string? filter)
        {
            try
            {
                using (conn = new MySqlConnection(SqlConnectionString.ConnectionString))
                {
                    ...
                    {
                        sqlCommand = $"SELECT * FROM Orders ORDER BY id DESC LIMIT 10 OFFSET @PageNumber";
                        TotalRecord = conn.Query<Order>("Select * from Orders").Count();
                    }
                    else
                    {
                        sqlCommand = $"SELECT * FROM Orders WHERE name LIKE CONCAT('%',@filter,'%') OR status LIKE CONCAT('%',@filter,'%') ORDER BY id DESC LIMIT 10 OFFSET @PageNumber";
