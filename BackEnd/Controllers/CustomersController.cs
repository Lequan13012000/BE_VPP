        ...
        public IActionResult Pagination(int PageNumber, string? filter)
        {
            try
            {
                using (conn = new MySqlConnection(SqlConnectionString.ConnectionString))
                {
                    ...
                    {
                        sqlCommand = $"SELECT * FROM Customer ORDER BY id DESC LIMIT 20 OFFSET @PageNumber";
                        TotalRecord = conn.Query<Customer>("Select * from Customer").Count();
                    }
                    else
                    {
                        sqlCommand = $"SELECT * FROM Customer WHERE name LIKE CONCAT('%',@filter,'%') OR email LIKE CONCAT('%',@filter,'%') ORDER BY id DESC LIMIT 20 OFFSET @PageNumber";
